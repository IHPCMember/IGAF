...
from data_loader import MVTecADTestDataset
...
def test(obj_names, mvtec_path, checkpoint_path, base_model_name, saveimages):
    ...
    for obj_name in obj_names:
        ...
        model = IGAFNet(in_channels=1, out_channels=3)
        model.load_state_dict(torch.load(os.path.join(checkpoint_path, run_name + ".pckl"), map_location='cuda:0'))
        model.cuda()
        model.eval()

        dataset = MVTecADTestDataset(mvtec_path + obj_name + "/test/", resize_shape=[img_dim, img_dim])
        ...
        with torch.no_grad():
            ...
            for i_batch, sample_batched in enumerate(dataloader):
                ...
                if saveimages:
                    ...
                    savefig(gray_batch, segresult, truemaskresult, f'{savepath}/{obj_name}/' + f'segresult{i}.png',
                            gray_rec)
                    i = i + 1
        ...
        save_log_path = './logs/mvtec_all_class_noDTD_log.log'
        set_logger1(save_log_path)
        logging.basicConfig(format='%(asctime)s %(message)s', level=logging.INFO)
        logging.info(
            'class_name:\t{:s} AUC Image:\t{:.4f} AUC Pixel:\t{:.4f} PRO:\t{:.4f}'.format(obj_name, auroc, auroc_pixel,
                                                                                          au_pro))
